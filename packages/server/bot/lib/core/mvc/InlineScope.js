...
class InlineScope {
  ...
  answer(results, options, callback) {
    ...
    this._api
      .answerInlineQuery(this._inlineQuery.id, results, options)
      .then(() => {
  ...
  answerPaginated(results, answersPerPage, callback) {
    ...
    this._waitingQueries[
      this._inlineQuery.query + ':' + this._inlineQuery.from.id
    ] = $ => {
      $.answerPaginated(
        results.slice(answersPerPage),
        answersPerPage,
        callback
      );
    };
