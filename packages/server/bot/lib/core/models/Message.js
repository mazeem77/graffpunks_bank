...
class Message {
  ...
  constructor(
    ...
    (this._pinnedMessage = pinnedMessage), (this._invoice = invoice);
  ...
  static deserialize(raw) {
    return new Message(
      ...
      raw['photo']
        ? raw['photo'].map(item => PhotoSize.deserialize(item))
        : null,
      ...
      raw['left_chat_member']
        ? User.deserialize(raw['left_chat_member'])
        : null,
      raw['new_chat_title'] ? raw['new_chat_title'] : null,
      raw['new_chat_photo']
        ? raw['new_chat_photo'].map(item => PhotoSize.deserialize(item))
        : null,
      ...
      raw['successful_payment']
        ? SuccessfulPayment.deserialize(raw['successful_payment'])
        : null
    );
  }
  ...
  serialize() {
    return {
      ...
      forward_from_chat: this.forwardFromChat
        ? this.forwardFromChat.serialize()
        : undefined,
      ...
      reply_to_message: this.replyToMessage
        ? this.replyToMessage.serialize()
        : undefined,
      edit_date: this.editDate ? this.editDate : undefined,
      text: this.text ? this.text : undefined,
      entities: this.entities
        ? this.entities.map(item => item.serialize())
        : undefined,
