  ...
  static deserialize(raw) {
    return new Update(
      ...
      raw['edited_channel_post']
        ? Message.deserialize(raw['edited_channel_post'])
        : null,
      raw['inline_query'] ? InlineQuery.deserialize(raw['inline_query']) : null,
      raw['chosen_inline_result']
        ? ChosenInlineResult.deserialize(raw['chosen_inline_result'])
        : null,
      raw['callback_query']
        ? CallbackQuery.deserialize(raw['callback_query'])
        : null,
      raw['pre_checkout_query']
        ? PreCheckoutQuery.deserialize(raw['pre_checkout_query'])
        : null
    );
  }
