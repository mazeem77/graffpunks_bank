...
const WebhookUpdateHandler = require('./updateFetchers/WebhookUpdateHandler');
...
const TextArrayCommand = require('./routing/commands/TextArrayCommand');
const RegexpCommand = require('./routing/commands/RegexpCommand');
const ParamsCommand = require('./routing/commands/ParamsCommand');
...
class Telegram {
  ...
  constructor(token, options) {
    ...
    this._payments = options.payments || {};

    this._cpus = os.cpus();
    this._workersCount = options.workers;

    this._ipc = new TelegramIPC();

    this._telegramDataSource = new TelegramDataSource(
      new TelegramApi(token, this._logger, this._payments),
      new TelegramRouter(),
      this._logger,
      new TelegramSessionStorage(this._storage),
      this._localization,
      this._ipc,
      this._files
    );
    ...
    if (options.updateFetcher) this._updatesFetcher = options.updateFetcher;
    else if (options.webhook && options.webhook.use) {
      if (options.webhook.useHandler) {
        this._updatesFetcher = new WebhookUpdateHandler(
  ...
  _checkNodeVersion() {
    if (process.version.replace('v', '').split('.')[0] < 6) {
      this._logger.error({
        'Fatal error':
          'Node version must be 6 or greater, please update your Node.js'
      });
  ...
  _setup() {
    if (cluster.isMaster) this._master();

    if (cluster.isWorker) this._worker();
  }
  ...
  _master() {
    ...
    if (this._workersCount > 0) {
      ...
      this._updateProcessor = new UpdateProcessorsManager(
        this._telegramDataSource
      );
  ...
  restartWorkers() {
    ...
    for (const pid in this._workers) {
      if (this._workers[pid]) this._workers[pid].kill();
    }
  }
  ...
  _runWorkers() {
    ...
    cluster.on('online', w =>
      this._logger.log({ Telegram: `Worker started at ${w.process.pid} PID` })
    );
    ...
    cluster.on('exit', (worker, code, signal) => {
      ...
      this._logger.log({
        Telegram: `Worker ${
          worker.process.pid
        } died with code: ${code}, and signal: ${signal}, Starting a new worker`
      });
      this._runWorker();
  ...
  /**
   *
   * @returns {UpdatesFetcher}
   */
  get updatesFetcher() {
    return this._updatesFetcher;
  }
   ...
   */
  get files() {
    return this._telegramDataSource.files;
  }
  ...
  _processUpdates(updates) {
    if (cluster.isMaster && this._workersCount > 0) {
      updates.forEach(u => {
        let worker;
        ...
        if (u.message && this._waitingUpdates[u.message.chat.id] != null)
          ...
          u.callbackQuery &&
          this._waitingCallbacks[u.callbackQuery.data] != null
        )
          worker = this._waitingCallbacks[u.callbackQuery.data];
        else worker = this._pickRandomWorker(); //pick random worker for update
  ...
  _pickRandomWorker() {
    const pids = Object.keys(this._workers).filter(
      pid => this._workers[pid] != null
    );
