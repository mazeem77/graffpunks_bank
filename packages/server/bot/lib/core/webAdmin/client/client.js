...
function updateData() {
  getRequest('/logs', function(logs) {
    if (logsTextArea.value != logs) {
      var willScroll =
        logsTextArea.scrollHeight - logsTextArea.scrollTop <= logsHeight;
      logsTextArea.value = logs;
      if (willScroll) logsTextArea.scrollTop = logsTextArea.scrollHeight;
    }
  });
...
function renderHealth(stats) {
  return (
    ...
    '</div>' +
    '<br>' +
    '<div>Requests by workers: ' +
    renderRequestsByWorkers(stats.requestsForWorkers) +
    '</div>' +
    '<br>' +
    '<div>Workers status: ' +
    renderWorkersStatus(stats.workersStatus) +
    '</div>'
  );
}
