...
class Ivan {
  ...
  constructor(storage, localizations) {
    ...
    localizations.forEach(localization => {
      if (!this._checkLocalization(localization))
        throw `Wrong localization: ${localization}`;
   ...
   */
  forUser(userId) {
    return this.getLanguageForUser(userId).then(lang => {
  ...
  getLanguageForUser(userId) {
    return this._storage.get(LOCALIZATION_STORAGE, userId).then(user => {
      if (user.lang) {
        return user.lang;
      } else {
  ...
  localizationForLanguage(lang) {
    let loc = this._localizations.find(
